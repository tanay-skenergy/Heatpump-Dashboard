...
export default function Dashboard() {
  ...
  const [targetInput, setTargetInput] = useState("55"); // State to handle the text input
  ...
  const [timeFilter, setTimeFilter] = useState('all'); // Options: '24h', '7d', 'custom'
...
const { error } = await supabase
 ...
 useEffect(() => {
  ...
  // Updated calculation using targetInput
const minsToHeat = (Number(targetInput) - data.waterTemp) > 0 && powerInputKW > 0 
  ? Math.round((200 * 4.186 * (Number(targetInput) - data.waterTemp)) / (powerInputKW * 60)) 
  : 0;
  ...
  <div>
    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Target Set Point (°C)</span>
    <div className="flex items-center gap-2 mt-4">
      <input 
        type="number" 
        value={targetInput}
        onChange={(e) => setTargetInput(e.target.value)}
        className="bg-transparent text-7xl font-black text-white w-full focus:outline-none focus:text-cyan-400 transition-colors"
        placeholder="55"
      />
  ...
  <div className="border-t border-slate-800 pt-6">
    ...
    <p className="text-xl font-bold text-cyan-400 mt-2">
      {minsToHeat > 0 ? `${minsToHeat} Minutes` : "Target Reached"}
    </p>
  </div>
</Card>
            </div>
...
<div className="flex flex-wrap items-center gap-4 mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
  <div className="flex gap-2">
    {['all', '24h', '7d', 'custom'].map((f) => (
      <button 
        key={f}
        onClick={() => setTimeFilter(f)} 
        className={`px-4 py-2 rounded-lg text-sm font-medium transition capitalize ${
          timeFilter === f ? 'bg-[#0f172a] text-white' : 'bg-white text-slate-600 border hover:bg-slate-50'
        }`}
      >
        {f === 'all' ? 'All Time' : f === '24h' ? 'Last 24h' : f === '7d' ? 'Last 7 Days' : 'Custom'}
      </button>
    ))}
  </div>
  ...
  {timeFilter === 'custom' && (
    <div className="flex items-center gap-2">
      <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm" />
      <span className="text-slate-400 text-sm">to</span>
      <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm" />
    </div>
  )}
</div>
